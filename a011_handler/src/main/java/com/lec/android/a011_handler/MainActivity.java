...
/*
레이아웃부분에 "mOnClick"을 정해줘야한다.
Main---->작업 Thread (가능)
Main<----작업 Thread (불가능) -> Handler을 통해 접근가능
 */
/** Thread 사용
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        thread1.setDaemon(true); //메인스레드가 죽으면 같이 죽어. 종료 동기화.
    ...
    class BackThread extends Thread{
        ...
        public void run() {
            while(true){
                backValue1++; //작업 스레드값 증가
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                //tvBackValue1.setText(""+backValue1); //""+backValue1이게 문자열로 만들어주는것
