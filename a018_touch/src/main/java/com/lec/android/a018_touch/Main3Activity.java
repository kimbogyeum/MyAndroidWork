...
public class Main3Activity extends AppCompatActivity {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK){ //event.getAction()-정수값 &-비트연산자

            case MotionEvent.ACTION_DOWN: //한개 포인트에 대한 DOWN 이 발생했을때
                event.getPointerId(0); //터치한 순간에 부여되는 포인트 고유번호
                x[0]=(int)(event.getX()); //매개변수 없는건 무조건 0번째
                y[0]=(int)(event.getY());
                result="싱글터치: \n";
                result+="("+x[0]+","+y[0]+")";
                break;
             case MotionEvent.ACTION_POINTER_DOWN: //두개이상의 포인트에 대한 DOWN이 발생했을때
                result="멀티터치: \n";
